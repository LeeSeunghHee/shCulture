...
<div id="joinBox">
	...
	<div id="joinCheckBox">
		<form method="POST" id="joinForm" enctype="multipart/form-data">
			<p class="smp">프로필</p>
			<div class="profile_picBox">
				<img class="previewProfic"
					src="${cpath }/resources/image/basicProfile.png"
					onclick="inputProfile()">
			</div>
			<input class="inputProfic" name="uploadProfile" type="file"
				accept="image/*" hidden="true" onchange="profileHandler()">
			<p class="smp">아이디</p>
			...
			<p class="smp">이메일</p>
			<input class="joinEmail" type="email" name="email" placeholder="이메일"
				onkeyup="checkEmail()" autocomplete="off" required="required">
			<button id="checkE" onclick="verification_codeSend(event)">인증번호 전송</button>
			<div id="accessBox">
				<input type="text" id="accessNum">
				<button id="accessBtn" onclick="verification_codeCheck(event)">본인인증</button>
			</div>
			<p class="emt">중복된 이메일입니다.</p>
			<p class="checkMessage"></p>
			<input class="memberT" type="text" value="M" name="memberType"
				hidden="true"> <input onclick="submitBtn(event)"
				id="joinBtn" type="submit" value="회원가입">
		</form>
	</div>
</div>
...
async function checkEmail() {
	var email = document.querySelector('.joinEmail[name="email"]').value;
	var checkBtn = document.getElementById("checkE");
...
async function verification_codeSend(event) {
    ...
    accessBtn.style.visibility = "visible"; 
    const email = document.querySelector('.joinEmail[name="email"]').value;
...
function verification_codeCheck(event) {
	event.preventDefault();
	var inputNum = document.getElementById('accessNum').value;
	var message = document.querySelector('.checkMessage');
...
async function submitBtn(event) {
	...
	const checkNum = document.getElementById('accessNum').value;

	
	if(checkNum !== ranNumber){
		alert('인증번호를 확인해주세요');
