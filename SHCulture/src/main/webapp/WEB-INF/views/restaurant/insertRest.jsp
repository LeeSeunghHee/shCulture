...
<div id="insertBox">
	<form method="POST" enctype="multipart/form-data" id="InsertRest">
		...
		<div class="detailR">
			<input type="text" name="restName" placeholder="업체명" required="required" autocomplete="none">
		</div> 
		<div class="imgSpan">
			<span class="restImg">매장 이미지 등록&ensp;</span>
			<input type="file" name="uploadRest" accept="image/*" required="required" onchange="previewImg(event)">
		</div>
		...
		<div class="detailR"><input type="text" class="restNum" name="restNum" onchange="checkHyphen()" placeholder="매장번호" required="required" autocomplete="none"></div> 
		<div class="restD">운영시간<span class="pilsu">(*)</span></div>
			<div class="infoHour">(24시일 경우 둘 다 오전으로 등록)</div>
			<span class="detailR"><input type="time" name="startTime" class="startT"></span> 
			<span class="wave">~&nbsp;</span> 
			<span class="detailR"><input type="time" name="endTime" class="startT" onchange="checkTime()"></span> 
		<details id="sumLastInput">
			<summary>⏰브레이크타임/라스트오더</summary>
			<ul>
				<li>
					<div class="restTi">브레이크타임</div>
					<span class="detailR"><input type="time" name="breakTiS" class="detailTi"></span>
					<span class="wave">~&nbsp;</span>
					<span class="detailR"><input type="time" name="breakTiE" class="detailTi"></span>
				</li>
				<li>
					<div class="restTi">라스트오더</div>
					<span class="detailR"><input type="time" name="lastOrder" class="detailTi"></span>
				</li>
			</ul>
		</details>
		<div class="restD">최대 수용 인원</div>
		<div class="detailR"><input type="number" name="maxPeople" min="1"></div>
		...
		<div class="detailR">
			...
			<div class="detailR">
				<textarea name="restInfo" placeholder="매장 소개를 적어주세요"></textarea>
			</div>			
			<div class="restD">주소<span class="pilsu">(*)</span></div>
			<div class="detailQ">
				<input type="text" id="postalcode" name="postalCode" placeholder="우편번호"> 
				<input type="button" onclick="sample4_execDaumPostcode()" value="우편번호 찾기"><br>
				<input type="text" id="roadAddr"  name="roadName" placeholder="도로명주소" autocomplete="none" required="required">
				<input type="text" id="localAddr" name="localNum" placeholder="지번주소" autocomplete="none" required="required">
				<input type="text" id="detailAddr" name="detailL" placeholder="상세주소" autocomplete="none">
				<input type="text" name="gun" hidden="true"> 
			</div>
	...
	<div id="NextBtn"><button id="BtnInsertR" onclick="InsertRHandler(event)">다음</button></div>
</div>
...
<script>
	...
	function addSNS() {
	    ...
	    newAddButton.addEventListener('click', addSNS);
	...
	// snsList 정리 및 편의시설 정리 그리고 submit
 	async function InsertRHandler(event) {
		...
		const url = cpath + '/restaurant/insertRest';
